...
## DA Model Inventory (Current Status — 2026-05-21)

| Model | Test RMSE | Status | Notes |
|-------|-----------|--------|-------|
| **da_v2 XGBoost** (`new_pipeline/build_da_v2.py`) | **140.88 W/m²** | ✅ **Active — Official DA Baseline** | 55 DA-valid features, no leakage, 19Q trained |
| DA-Exp1 XGBoost (Optuna) | 140.18 W/m² | ✅ Active — Experimental | ≈da_v2, high MBE (+9.94), no AgACI |
| raw_19q / xgbq_19q | 124.92 W/m² | ⛔ **DEPRECATED — Contains Leakage** | 8 unlagged station obs at target hour; column-name mismatch caused lag to silently skip. See `DA_BASELINE_TRANSITION_REPORT_2026-05-21.md`. |

**Valid DA RMSE baseline for paper citation: da_v2 = 140.88 W/m²**  
Do not cite raw_19q 124.92 W/m² as a leakage-free number.
...
## Sequential Day-Ahead MILP — Layer B Results (2026-03-31)

Per `0331_MILP_without_REP_SPEC_final.docx` (March 29) and `0331_bridge__SPEC_final.docx` (March 30), the formal mainline is a **two-layer hierarchical framework**:

| Layer | What it does | Output |
|-------|-------------|--------|
| **Layer A** (done) | Annual fixed design — solves once for CC\*, P\_B\*, E\_B\* over the full year | `milp_outputs/case_summary_fullyear.csv` |
| **Layer B** (new) | Sequential day-ahead daily MILP — 365 × 24h solves, fixed design, D-1 20:00 gate | `milp_outputs/da/` |

### Five spec rules enforced in Layer B

1. **Day-ahead, not annual** — each solve covers exactly 24 hours for the next calendar day
2. **D-1 20:00 information boundary** — only forecast data frozen before that gate is used; realized truth is never seen by the solver
3. **Scenario-invariant battery control** — `u[t]`, `P_ch[t]`, `P_dis[t]`, `E[t]` are the same across all scenarios; scenarios only affect settlement variables (`P_grid[ω,t]`, `P_pv_load[ω,t]`, `E_g[ω,t]`)
4. **One solve per day** — not rolling hourly MPC, not one-shot annual; exactly one 24h MILP per calendar day
5. **Serialized state carry-forward** — end-of-day SOC, Green SOC, and month-to-date demand peak propagate to the next day; month-to-date demand resets at month boundaries

### Layer A Design Results (visible summary)

`milp_outputs/da/layer_a_annual_design_results.csv` — full Layer A results for all 5 cases in one place:

| Case | CC\* (kW) | P\_B (kW) | E\_B (kWh) | Solve Obj (M NTD) | Solve (s) | AEC\_inv | AEC\_ene | AEC\_over | RE% | Layer A Replay (M NTD) |
|------|----------|----------|-----------|-------------------|-----------|---------|---------|---------|-----|----------------------|
| C0 | 3,215.2 | 1,143.6 | 7,127.9 | 96.78 | 14.7 | 6.63 | 75.36 | 0.43 | 20.0% | 97.21 |
| C1 | 3,205.2 | 1,154.6 | 7,250.1 | 96.51 | 48.2 | 6.73 | 75.13 | 0.36 | 20.0% | 97.21 |
| C2 | 3,257.4 | 1,178.8 | 8,168.9 | 97.23 | 39.0 | 7.45 | 74.38 | 0.75 | 20.0% | 97.25 |
| C3 | 3,252.5 | 1,207.0 | 8,110.7 | 97.44 | 37.6 | 7.44 | 74.47 | 0.91 | 20.0% | 97.25 |
| PI | 3,202.0 | 1,129.7 | 7,308.7 | 97.21 | 13.7 | 6.75 | 75.56 | 0.36 | 20.0% | 97.20 |

### Layer B Annual Results

Generated by `notebooks_milp/milp_da_sequential.py` using Layer A sizing from `case_summary_fullyear.csv`.

| Case | CC\* (kW) | P\_B (kW) | E\_B (kWh) | Replay Total (M NTD) | Over-contract (M NTD) | RE% | Worst Month (M NTD) |
|------|----------|----------|-----------|---------------------|----------------------|-----|---------------------|
| C0 | 3,215.2 | 1,143.6 | 7,127.9 | **93.837** | 0.434 | 16.0% | 10.565 |
| C1 | 3,205.2 | 1,154.6 | 7,250.1 | **93.695** | 0.358 | 16.0% | 10.550 |
| C2 | 3,257.4 | 1,178.8 | 8,168.9 | **93.892** | 0.746 | 16.2% | 10.549 |
| C3 | 3,252.5 | 1,207.0 | 8,110.7 | **94.047** | 0.913 | 16.2% | 10.566 |
| **PI** | 3,202.0 | 1,129.7 | 7,308.7 | **93.755** | 0.358 | 16.0% | 10.565 |

PI is the oracle lower bound (perfect forecast). PI < C0 confirms that forecast error carries a real operational cost.

Cost components (M NTD):

| Case | Inv | Energy | Basic | Over | T-REC |
|------|-----|--------|-------|------|-------|
| C0 | 6.629 | 75.672 | 7.527 | 0.434 | 3.575 |
| C1 | 6.734 | 75.514 | 7.510 | 0.358 | 3.580 |
| C2 | 7.446 | 74.665 | 7.632 | 0.746 | 3.402 |
| C3 | 7.435 | 74.680 | 7.621 | 0.913 | 3.398 |
| PI | 6.749 | 75.572 | 7.502 | 0.358 | 3.574 |

### Layer B Output Files

```
milp_outputs/da/
├── layer_a_annual_design_results.csv      # Layer A: design variables + full cost breakdown (5 cases)
├── layer_a_frozen_design_{C0..PI}.json   # Module 2 frozen design artifacts (5 cases)
├── daily_plan_trace_{C0..PI}.parquet     # 8760 rows: hourly battery plan (scenario-invariant)
├── daily_settlement_trace_{C0..PI}.parquet # 8760 rows: realized settlement vs truth
├── rolling_state_trace_{C0..PI}.parquet  # 365 rows: SOC/GreenSOC/D_mth start+end per day
├── da_daily_solve_log_{C0..PI}.csv       # 365 rows: gate metadata + solve stats per day
├── replay_monthly_bill_{C0..PI}.csv      # 12 rows: monthly energy/basic/over/total bill
├── replay_summary_{C0..PI}.json          # annual totals per case
└── da_case_comparison.csv                # 5-case comparison table (C0–C3 + PI)
```

### Bridge rolling_da_input packages

`bridge_outputs_fullyear/rolling_da_input_*.parquet` — 4 files, one per case combination, produced by `notebooks_bridge/bridge_full_year.py`. Schema includes `issue_day` (D-1) and `gate_time_local = "20:00"` to make the day-ahead information boundary auditable.
...
### Rolling Day-Ahead Two-Layer Framework — Final Results

**Framework:** Layer A (annual design via Nelder-Mead outer loop + sequential 365-day MILP inner evaluator) → Layer B (fixed-design realized-truth replay). Window: 2024-11-01 to 2025-11-01.

**Load uncertainty:** 5% uniform MAPE (replaces Pieter seasonal MAPE). **Early stop:** patience=30, tol=0.005M.

#### Layer A — Annual Design (Solve-World)

| Case | Description | CC\* (kW) | P_B\* (kW) | E_B\* (kWh) | Solve J(x) (M NTD) | Iters |
|------|-------------|-----------|-----------|------------|-------------------|-------|
| C0 | Det PV + Det Load | 3,096 | 1,444 | 9,990 | 75.71 | 13 |
| C1 | Prob PV + Det Load | 3,210 | 1,347 | 9,567 | 75.58 | 87 |
| C2 | Det PV + Load Unc (5%) | 3,217 | 1,702 | 8,463 | 76.46 | 76 |
| C3 | Prob PV + Load Unc (5%) | 3,225 | 1,650 | 8,125 | 76.68 | 42 |
| PI | Perfect Information (benchmark) | 3,202 | 1,130 | 7,309 | 77.77 | — |

#### Layer B — Fixed-Design Replay (Realized Truth)

Annual cost breakdown (M NTD). T-REC covers gap from physical RE% to the mandatory 20% target.

| Case | Energy | Basic | Over-Contract | T-REC | CAPEX | **Replay Total** | Physical RE% | Over Months | Worst Month |
|------|--------|-------|---------------|-------|-------|-----------------|-------------|-------------|-------------|
| C0 | 73.40 | 7.26 | 0.21 | 5.27 | 9.11 | **97.59** | 14.65% | 4 | 10.23 |
| C1 | 73.54 | 7.52 | **0.06** | 5.24 | 8.68 | **97.36** | 14.68% | **2** | 10.25 |
| C2 | 74.27 | 7.54 | 0.25 | 5.25 | 8.27 | **97.76** | 14.67% | 3 | 10.40 |
| C3 | 74.53 | 7.56 | 0.29 | 5.25 | **7.96** | **97.71** | 14.67% | 3 | 10.45 |
| PI | 75.57 | 7.50 | 0.36 | 3.57 | 6.75 | **93.75** | 16.03% | 4 | 10.57 |

All cases achieve RE >= 20% after T-REC procurement. T-REC unit cost: 4.63 NTD/kWh.

**Key findings:**
- **C1 lowest replay cost (97.36M):** Probabilistic PV reduces over-contract from 0.21M (C0) to 0.06M and halves over-months (4 → 2)
- **C3 beats C2 (97.71M vs 97.76M):** Joint PV+load uncertainty achieves lowest CAPEX (7.96M) via smaller BESS
- **PI benchmark 93.75M:** 3.6–4.0M below mainline cases — value of perfect foresight
- RE physically attained: 14.65–16.03%; gap to 20% covered by T-REC (~5.2–5.3M for C0–C3)

### Chapter 4 Results Package

Formal spec-named figures and tables in `results/ch4/`. Generated by `notebooks_milp/ch4_results.py` from rolling DA Layer A/B outputs.
...
│   └── S-F1..S-F4   # Sensitivity heatmaps (N×K AEC, delta-baseline, stability)
...
└── qa/              # qa_report.json (0 hard fails, 5 warnings)
```
