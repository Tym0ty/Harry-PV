...
Results from `notebooks_milp/milp_degraded_det.py` + `notebooks_milp/milp_complete_batches.py`. Uses old Taipower meter readings, PV=2,687 kW, RE20 ON. The **deterministic** PV forecast (C0, C2) is degraded with a **2× over-prediction bias** to simulate a lower-quality point forecast. Probabilistic cases (C1, C3) use unchanged scenarios. All cases optimize freely (no cross-case sizing bounds).

**Motivation:** With an unbiased det forecast, C0 and C1 replay costs converge to the same value (gap ~0.01M). By introducing forecast degradation, we demonstrate the meaningful cost advantage that probabilistic forecasting provides when the deterministic baseline is imperfect.

#### Sizing (Degraded Det, bias=2.0×)

| Case | PV Input | Load Input | CC (kW) | P_B (kW) | E_B (kWh) | E/P |
|------|----------|------------|---------|----------|-----------|-----|
| C_PI | Perfect | Perfect | 3,202 | 1,130 | 7,309 | 6.5 |
| C0_deg | Det ×2 bias | Det | 2,914 | 1,466 | 8,145 | 5.6 |
| C1 | Probabilistic | Det | 3,212 | 1,147 | 7,140 | 6.2 |
| C2_deg | Det ×2 bias | Perturbed | 3,064 | 1,543 | 8,705 | 5.6 |
| C3 | Probabilistic | Perturbed | 3,393 | 1,193 | 7,478 | 6.3 |
...
| C0_deg | 74.92 | 6.83 | **0.83** | 5.20 | 2.01 | **7.76** | **97.55** | 20.0 | 4 | 10.58 |
| C1 | 75.68 | 7.53 | **0.35** | 5.20 | 1.81 | **6.64** | **97.21** | 20.0 | 4 | 10.58 |
| C2_deg | 74.29 | 7.18 | **0.39** | 5.20 | 2.16 | **8.26** | **97.49** | 20.0 | 4 | 10.43 |
| C3 | 75.26 | 7.95 | **0.09** | 5.20 | 1.91 | **6.95** | **97.35** | 20.0 | 2 | 10.50 |
...
- **C0_deg − C1 gap = +0.34M**: With an over-optimistic PV forecast, C0 chooses a smaller CC (2,914 vs 3,212 kW), expecting solar to cover more peak demand. In replay, actual PV is only half the forecast → over-contract jumps to 0.83M vs 0.35M for C1 (+0.48M), and C0 over-invests in BESS (+0.37M investment). The basic charge savings from the lower CC (−0.70M) only partially offset these costs.
- **C2_deg − C3 gap = +0.14M**: Under load perturbation, biased det (C2_deg) over-contracts at 0.39M vs prob (C3) at 0.09M. C3 also reduces worst-month bill (10.43M vs 10.50M). The perturbed load provides a stronger over-contract buffer than unperturbed.
...
| C1 | Probabilistic | Det | 3,263 | 1,109 | 7,128 | 6.4 |
...
| C1 | 75.69 | 7.65 | **0.28** | 5.20 | 1.81 | **6.59** | **97.21** | 20.0 | 4 | 10.58 |
...
### Table 3 — Cheat (bias=2×), RE20 ON, PV = 2,687 kW

Det forecast (C0, C2) inflated by 2×. C1 and C3 are probabilistic (unchanged). C_PI identical to Table 1.
...
| C1 | Probabilistic | Det | 3,901 | 604 | 4,305 | 7.1 |
| C2_deg | Det ×2 bias | Perturbed | 3,848 | 764 | 5,444 | 7.1 |
| C3 | Probabilistic | Perturbed | 4,099 | 654 | 4,658 | 7.1 |
...
| Setting | Best Case | Worst Det Gap to Prob | C0(det) vs C1(prob) | C0_deg vs C1 gap |
|---------|-----------|----------------------|---------------------|------------------|
| RE20, PV=2687, no cheat | C1/C0 tied (97.21M) | C2 vs C3: +0.00M | 0.00M | — |
| RE20, PV=2687, cheat | C1 (97.21M) | C2_deg vs C3: +0.14M | **+0.34M** | +0.34M |
| No RE20, PV=515, no cheat | C0/C1/PI tied (103.80M) | C2 vs C3: +0.01M | 0.00M | — |
| No RE20, PV=515, cheat | C1/PI tied (103.80M) | C2_deg vs C3: -0.07M | **+0.08M** | +0.08M |

- **Bias amplifies probabilistic advantage**: Without bias, C0≈C1 (gap ~0.01M). With 2× bias, C0_deg vs C1 = +0.34M (RE20) or +0.08M (no RE20). The advantage is larger with RE20 because the T-REC constraint forces higher BESS sizing, amplifying the consequence of mis-sized CC.
- **C3 (prob+pert) is the most robust**: Lowest over-contract in all settings (0.09M at RE20, 0.12M at no-RE20). Load perturbation drives CC higher, which directly hedges over-contract risk.
- **At 515 kW, bias impact is muted**: Smaller PV contribution means det vs prob distinction matters less — over-contract is dominated by load uncertainty rather than PV forecast accuracy.
