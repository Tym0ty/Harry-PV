...
```
milp_outputs/da/
├── daily_plan_trace_{C0..C3}.parquet      # 8760 rows: hourly battery plan (scenario-invariant)
├── daily_settlement_trace_{C0..C3}.parquet# 8760 rows: realized settlement vs truth
├── rolling_state_trace_{C0..C3}.parquet  # 365 rows: SOC/GreenSOC/D_mth start+end per day
├── replay_monthly_bill_{C0..C3}.csv      # 12 rows: monthly energy/basic/over/total bill
├── replay_summary_{C0..C3}.json          # annual totals per case
└── da_case_comparison.csv                # 4-case comparison table
```
