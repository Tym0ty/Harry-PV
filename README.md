...
Results from `notebooks_milp/milp_degraded_det.py`. Uses old Taipower meter readings (same as Config 2), PV=2,687 kW, RE20 ON. The C0 deterministic PV forecast is degraded with a **2× over-prediction bias** to simulate a lower-quality point forecast. C1 probabilistic scenarios are unchanged. C1 optimizes freely (no sizing bounds tied to C0).
...
|------|---------|----------|-----------|-----|
| C0_deg | 2,914 | 1,466 | 8,145 | 5.6 |
| C1 | 3,212 | 1,147 | 7,140 | 6.2 |

#### Replay Cost Breakdown (M NTD)

| Case | Energy | Basic | Over-Contract | Green/T-REC | Degradation | Investment | **Total** | RE% | Over Months |
|------|--------|-------|---------------|-------------|-------------|------------|-----------|-----|-------------|
| C0_deg | 74.92 | 6.83 | 0.83 | 5.20 | 2.01 | 7.76 | **97.55** | 20.0 | 4 |
| C1 | 75.68 | 7.53 | **0.35** | 5.20 | 1.81 | **6.64** | **97.21** | 20.0 | 4 |

**C0_deg − C1 gap = +0.34M NTD**

#### Key Findings

- **C1 saves 0.34M/yr vs degraded C0**: With an over-optimistic PV forecast, C0 chooses a smaller CC (2,914 vs 3,212 kW), expecting solar to cover more peak demand. In replay, actual PV is only half the forecast → over-contract jumps to 0.83M vs 0.35M for C1 (+0.48M), and C0 over-invests in BESS (+0.37M investment). The basic charge savings from the lower CC (−0.70M) only partially offset these costs.
- **Attribution of the 0.34M gap**: Over-contract (+0.48M) + Investment (+1.12M) − Energy savings (−0.76M) − Basic savings (−0.70M) + Degradation (+0.20M) ≈ +0.34M.
- **Bias factor**: 2× over-prediction (det forecast inflated by 100%). This represents a scenario where the day-ahead PV point forecast is systematically too optimistic — plausible under poor NWP conditions or naive persistence-based forecasting.
